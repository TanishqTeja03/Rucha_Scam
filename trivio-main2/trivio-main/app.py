...
def get_questions():
    ...
    print(f"🔗 Received link header: {url}")
    ...
    for i, (chunk, timestamp) in enumerate(chunks):
        print(f"📚 Generating question for chunk {i+1}/{len(chunks)}")
        question_data = generate_questions_and_options(chunk)
        if question_data:
            question_data["timestamp"] = timestamp
            questions_data.append(question_data)
        else:
            print(f"⚠️ Failed to generate question for chunk {i+1}")

    print(f"✅ Returning {len(questions_data)} questions to the extension.")
